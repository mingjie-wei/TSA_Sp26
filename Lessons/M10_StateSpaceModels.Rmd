...
decompose_allsectors_price <- decompose(ts_electricity_price,"additive")
...
ts_price_train <- subset(ts_electricity_price,
                         end=(length(ts_electricity_price)-n_for))

ts_price_test <- subset(ts_electricity_price,
                        start=(length(ts_electricity_price)-n_for+1))
...
MEAN_seas <- meanf(y = ts_price_train, h = n_for)  
...
```{r}
SNAIVE_seas <- snaive(ts_price_train, h=n_for, holdout=FALSE)
...
### Model 3: SARIMA
```{r}
SARIMA_autofit <- auto.arima(ts_price_train)
...
```{r}
SSES_seas <- es(ts_price_train,model="ZZZ",h=n_for,holdout=FALSE)
plot(SSES_seas)
...
```{r}
SS_seas <- StructTS(ts_price_train,
...
```{r}
#Model 1: Arithmetic mean
MEAN_scores <- accuracy(MEAN_seas$mean,ts_price_test)  #store the performance metrics

#Model 2: Seasonal naive 
SNAIVE_scores <- accuracy(SNAIVE_seas$mean,ts_price_test)

# Model 3:  SARIMA 
SARIMA_scores <- accuracy(SARIMA_for$mean,ts_price_test)

# Model 4:  SSES
SSES_scores <- accuracy(SSES_seas$forecast,ts_price_test)

# Model 5:  BSM 
SS_scores <- accuracy(SS_for$mean,ts_price_test)
...
```{r}
autoplot(ts_price_test) +
  #autolayer(MEAN_seas, PI=FALSE, series="Mean") +
  #autolayer(SNAIVE_seas, PI=FALSE, series="Naive") +
  autolayer(SARIMA_for,PI=FALSE, series="SARIMA") +
  autolayer(SSES_seas$forecast,PI=FALSE, series="SSES") +
  autolayer(SS_for,PI=FALSE,series="BSM") + 
  xlab("Month") + ylab("Electricity Retail Price ($/kWh)") +
  guides(colour=guide_legend(title="Forecast"))
```
